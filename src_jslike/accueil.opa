type News.news =
  {int id, User.ref author, Date.date date_creation, Date.date date_edition,
   Date.date date_publication, string title, string content}

database News.news /skiutc/news[{id}]

database /* skiutc */ skiutc @mongo

module Accueil {
    function content() {
        if (User.is_team()) News.admin() else News.home()
    }
    function get_page() {
...
module News {
    ...
    function save(int ref, News.news news) {
        /skiutc/news[ref] <- {news with date_edition : Date.now()}
    }
    function ref_to_xhtml(int ref) {
        to_xhtml(/skiutc/news[ref])
    }
...
<br/>
<div style="text-align: right; width: 100%; font-size: small;">Le {
        printer(n.date_publication)}.</div>
</div>
    }
    function xhtml to_xhtml_admin(int key, News.news n) {
        ...
        function printer(Date.date date) {
            ...
            function manage_date() {
                d =
                    Date.to_human_readable(/skiutc/news[key]/date_publication);
                function validate() {
                    ...
                    new_date =
                        Date.of_human_readable({d with
                                                day : Int.of_string(day),
                                                month :
                                                  Date.Month.of_int(Int.of_string(month) -
                                                                    1),
                                                year : Int.of_string(year),
                                                h : Int.of_string(heure),
                                                min : Int.of_string(min)});
                    save(key,
                      {/skiutc/news[key] with date_publication : new_date});
...
<tr><td/><td><button onclick={
                     function(_) {
                     validate()
                     }}>Valider</button></td></tr>
</table>
</>;
            ...
            <span id={id ^ "_date"} onclick={function(_) {
                                             manage_date()
            }}>{echo(date)}</span>
        }
        function edit_title() {
            Dom.set_value(#{(id ^ "_title_edit")}, /skiutc/news[key]/title);
            Dom.hide(#{(id ^ "_title")});
            Dom.show(#{(id ^ "_title_edit")})
        }
        function save_title() {
            title = Dom.get_value(#{(id ^ "_title_edit")});
            save(key, {/skiutc/news[key] with ~title});
            Dom.transform([#{id ^ "_title"} = <> {title} </>]);
            Dom.hide(#{(id ^ "_title_edit")});
            Dom.show(#{(id ^ "_title")})
        }
        function edit_content() {
            Dom.set_value(#{(id ^ "_content_edit")},
              /skiutc/news[key]/content);
            Dom.hide(#{(id ^ "_content")});
            Dom.show(#{(id ^ "_content_edit")})
        }
        function save_content() {
            content = Dom.get_value(#{(id ^ "_content_edit")});
            save(key, {/skiutc/news[key] with ~content});
            Dom.transform([#{id ^ "_content"} = Xhtml.of_string_unsafe(content)]);
            Dom.hide(#{(id ^ "_content_edit")});
            Dom.show(#{(id ^ "_content")})
        }
        function delete() {
            Db.remove(@/skiutc/news[key]);
            Dom.remove(#{(id ^ "_bloc")});
            Dom.Notification.show("", "Supression",
              "Une news vient d'\195\170tre supprim\195\169 !");
            void
        }
        <div class="right_cellule" id={id ^ "_bloc"}>
<span id={id ^ "_title"} onclick={
        function(_) {
        edit_title()
        }}> {if (n.title == "") "Empty title"
        else n.title} </span>
<input id={id ^ "_title_edit"} style={[Css_build.display({
...
<textarea id={
        id ^ "_content_edit"} style={[Css_build.display({css_none})]} onblur={
        function(_) {
        save_content()
        }} rows="20" cols="90"/>
<br/>
<div style="text-align: right; width: 100%; font-size: small;">{
    ...
    function add_form() {
        ...
        Dom.Notification.show("", "Ajout",
          "Une news vient d'\195\170tre ajout\195\169 !");
        void
    }
    function home() {
        @todo
    ...
    function xhtml admin() {
        <>
         <button id="new_news" onclick={function(_) {
                                        add_form()
         }}>Ajouter une news</button>
{@todo}</>
    }
}

function void init_news() {
    if (Map.size(/skiutc/news) == 0) {
      News.news first =
          {
           author : User_data.mk_ref("mguffroy"),
           date_creation : Date.now(),
           date_edition : Date.now(),
           date_publication : Date.now(),
           title : "SkiUTC 2012 Arrive !!",
           content :
             Xhtml.to_string(<div class="info">Soyez patient... ;) </div>)
           };
      /skiutc/news[@todo] <- first;
      void
    } else {} {}
}
